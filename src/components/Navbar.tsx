...
export const Navbar = ({ onOpenModal }: NavbarProps) => {
    ...
    return (
        <>
            ...
            {/* Quitamos las transiciones del header para concentrarlas en el NAV y evitar lag */}
            <header className={`fixed w-full z-[140] flex justify-center top-0 px-4 sm:px-6 pointer-events-none`}>
                <nav
                    className={`w-full flex justify-between items-center transition-all duration-[400ms] ease-in-out pointer-events-auto will-change-transform ${scrolled && !mobileMenuOpen
                            ? 'max-w-5xl bg-slate-900/80 backdrop-blur-2xl border border-white/10 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.6)] py-3 px-5 sm:px-8 mt-3 sm:mt-6'
                            : 'max-w-7xl bg-transparent py-6 sm:py-8 px-2 sm:px-4 border-transparent shadow-none mt-0'
                        } ${mobileMenuOpen ? 'bg-transparent py-6 px-2 sm:px-4' : ''}`}
                >
                    {/* --- LOGO --- */}
                    <Link
                        to="/"
                    ...
                    {/* --- MENÚ DESKTOP --- */}
                    <div className="hidden lg:flex items-center gap-2 bg-white/[0.03] border border-white/5 p-1.5 rounded-full shadow-inner">
                        {navLinks.map((link) => {
